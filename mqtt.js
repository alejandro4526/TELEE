...
// Variables para guardar los estados de los LEDs
    ...
    recognition.onresult = (event) => {
        ...
        if (command.includes('prender el ventilador')) {
            console.log('Comando para encender el ventilador');
            client.publish('test/ventilador', 'on', { qos: 2, retain: true }, (err) => {
                if (err) {
            ...
            console.log('Comando para apagar el ventilador');
            client.publish('test/ventilador', 'off', { qos: 2, retain: true }, (err) => {
                if (err) {
                    console.error('Error al publicar el mensaje:', err);
                } else {
